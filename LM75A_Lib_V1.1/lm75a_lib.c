...
/* ═══════════════════════════════════════════════════════════════════════════
   Extended functions (v1.2) — config register, shutdown, alarm thresholds
   ═══════════════════════════════════════════════════════════════════════════ */

static void i2c_writePointer(uint8_t reg){
	while (I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY));
	I2C_GenerateSTART(I2C1, ENABLE);
	while (!I2C_GetFlagStatus(I2C1, I2C_FLAG_SB));
...
static int16_t i2c_read16(uint8_t reg){
	...
	while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED));
	int16_t msb = I2C_ReceiveData(I2C1);
	I2C_AcknowledgeConfig(I2C1, DISABLE);
	I2C_GenerateSTOP(I2C1, ENABLE);
	while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_BYTE_RECEIVED));
	int16_t lsb = I2C_ReceiveData(I2C1);
	return (msb << 8) | lsb;
}
...
uint8_t Lm75a_ReadConfig(void){
	i2c_writePointer(LM75A_REG_CONF);
	I2C_GenerateSTART(I2C1, ENABLE);
	while (!I2C_GetFlagStatus(I2C1, I2C_FLAG_SB));
	I2C_Send7bitAddress(I2C1, lm75_Address, I2C_Direction_Receiver);
	while (!I2C_CheckEvent(I2C1, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED));
...
void Lm75a_SetShutdown(uint8_t enable){
	uint8_t conf = Lm75a_ReadConfig();
	if (enable)
		conf |= LM75A_CONF_SHUTDOWN;
	else
		conf &= ~LM75A_CONF_SHUTDOWN;
	Lm75a_WriteRegister(LM75A_REG_CONF, conf);
}


static void i2c_write16(uint8_t reg, int16_t value){
	i2c_writePointer(reg);
...
static int16_t tempToReg(float temp){
	return ((int16_t)(temp * 2)) << 7;
}

static float regToTemp(int16_t raw){
	return 0.5f * (raw >> 7);
}


void Lm75a_SetAlarmThresholds(float t_os, float t_hyst){
	i2c_write16(LM75A_REG_TOS,   tempToReg(t_os));
	i2c_write16(LM75A_REG_THYST, tempToReg(t_hyst));
}


float Lm75a_ReadTos(void){
	return regToTemp(i2c_read16(LM75A_REG_TOS));
}


float Lm75a_ReadThyst(void){
	return regToTemp(i2c_read16(LM75A_REG_THYST));
}
